...
fn get_log_size() -> u64 {
    fs::metadata("/var/log/pacman.log").map(|m| m.len()).unwrap_or(0)
}

fn parse_log_entries(log_content: &str) -> HashMap<String, PackageInfo> {
    let re = Regex::new(r"\[([0-9T:+-]+)\] \[ALPM\] (installed|upgraded|removed) ([^\s(]+)").unwrap();
    ...
    log_content
        ...
        .filter_map(|line| {
            re.captures(line).map(|caps| {
                ...
                (pkg_name, PackageInfo { date: date_str, status })
            })
        })
        .collect()
}
...
fn main() {
    ...
    let mut cache_data: CacheData = if Path::new(cache_file).exists() {
        fs::read_to_string(cache_file)
            ...
            .unwrap_or(CacheData { pkg_hash: String::new(), last_log_size: 0, data: HashMap::new() })
    } else {
        CacheData { pkg_hash: String::new(), last_log_size: 0, data: HashMap::new() }
    };
    ...
    if cache_data.pkg_hash != current_pkg_hash || cache_data.last_log_size != current_log_size {
        ...
        let _ = fs::write(cache_file, serde_json::to_string_pretty(&cache_data).unwrap());
    }

    // Kombinera installerade paket med tidigare loggade paket (inkl. REM)
    let mut pkg_set: HashMap<String, (String, String)> = cache_data.data
        .iter()
        .map(|(k,v)| (k.clone(), (v.date.clone(), v.status.clone())))
        .collect();

    for pkg in &current_pkgs {
        pkg_set.insert(pkg.clone(), pkg_set.get(pkg)
            .cloned()
            .unwrap_or(("0000-00-00T00:00:00+0000".to_string(), "INS".to_string())));
    }

    // Konvertera till vektor och sortera på datum
    let mut pkg_list: Vec<(String, String, String)> = pkg_set.into_iter()
